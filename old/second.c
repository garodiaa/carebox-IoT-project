...
// Button pins from the wiring diagram:
// btn1 = 25, btn2 = 26, btn3 = 27
...
void checkButtons() {
  for (int i = 0; i < NUM_COMPARTMENTS; i++) {
    // Read button state with simple debounce
    ...
    */
    ...
    if (previousButtonState[i] == HIGH && state == LOW) {
      ...
      if (hadActiveReminder) {
        ...
        // No active reminder - show blue blink briefly to acknowledge button press
        setBlueBlink(i);
...
void setup() {
  ...
  for (int i = 0; i < NUM_COMPARTMENTS; i++) {
    ...
    Serial.printf("[Setup] Initializing compartment %d: R=%d, G=%d, B=%d, Button=%d\n",
                  i+1, redPins[i], greenPins[i], bluePins[i], reedPins[i]);
  ...
  // LED startup sequence - Shorter RGB test cycle for each compartment using our functions
  ...
  Serial.println("[Setup] Complete - RGB LEDs ready!");
  
  // Final startup indication - all LEDs white blink once
  Serial.println("[Setup] Final white flash confirmation");
  for (int i = 0; i < NUM_COMPARTMENTS; i++) {
    setWhite(i);
  }
  delay(500);
